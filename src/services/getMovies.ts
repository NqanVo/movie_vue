...
export const getMoviesPopular = async (currentPage: number = 1, language: string = `en-US`) => {
  try {
    const res = await axios.get(
      BASE_URL + `movie/popular?api_key=${API_KEY}&language=${language}&page=${currentPage}`,
      { timeout: timeout }
    )
    return res.data
  } catch (error) {
...
export const getMoviesTopRated = async (currentPage: number = 1, language: string = `en-US`) => {
  try {
    const res = await axios.get(
      BASE_URL + `movie/top_rated?api_key=${API_KEY}&language=${language}&page=${currentPage}`,
      { timeout: timeout }
    )
    return res.data
  } catch (error) {
...
export const getDetailMovie = async (idMovie: number | string, language: string = `en-US`) => {
  try {
    const res = await axios.get(
      BASE_URL + `movie/${idMovie}?api_key=${API_KEY}&language=${language}&append_to_response=videos`,
      { timeout: timeout }
    )
    return res.data
  } catch (error) {
